...
\begin{figure}[htbp]
    ...
    \textbf{(b)} Comparison of empirical coverage rates. The Gaussian baseline (yellow) fails to capture the true distribution's shape, resulting in unpredictable coverage (often over- or under-covering depending on the skew). In contrast, Conformal Prediction (green) achieves near-perfect validity (90.8\%) by relying on the empirical quantiles of the residuals rather than a density estimate.
    \textbf{(c)} Visualization of CP intervals for the first 50 test samples. The intervals (green bars) effectively contain the true targets (black crosses) even in the presence of asymmetric noise, with failures (red crosses) occurring at the expected rate of $\alpha=10\%$.}
    \label{fig:dist_free_experiment}
\end{figure}

The results, shown in Figure \ref{fig:dist_free_experiment}, underscore the necessity of distribution-free methods. Panel (a) reveals the significant discrepancy between the actual skewed residuals (blue histogram) and the best-fit Gaussian distribution (red dashed line). Because the baseline method assumes symmetry and thin tails, it cannot accurately model the risk. Consequently, as shown in Panel (b), the baseline achieves an arbitrary coverage rate that may not align with the safety requirement.

Conformal Prediction, however, remains unaffected by the skewness. By computing the nonconformity scores directly from the data (Panel c) and determining the threshold empirically, it achieves a coverage of 90.8\%, aligning almost perfectly with the target. This confirms that CP provides valid uncertainty quantification for arbitrary data distributions without requiring feature engineering or complex density estimation.
