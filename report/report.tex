...
To empirically validate this theoretical guarantee, particularly in data-scarce regimes, we conducted a comparative experiment using the \textit{Diabetes} dataset. We employed Gradient Boosting Quantile Regression (QR) as the base model to predict disease progression based on BMI and other features. The data was partitioned to simulate a small-sample setting with a calibration set of only $N_{\text{cal}}=50$ samples. We compared the ``Raw'' output of the quantile regressors against the CQR method, targeting a 90\% coverage rate ($\alpha=0.1$).
...
\begin{figure}[htb!]
    ...
    \textbf{(b)} Prediction intervals for random test samples. \textit{Parametric} and Bayesian methods produce overly wide, uniform intervals. MC Dropout produces narrow intervals that fail to cover the true label (black crosses). CP produces adaptive, valid intervals.
